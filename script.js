...
suggestionButton.addEventListener('click', () => {
  ...
  const arrayOfGenres = data.map(element => element.firstChild.nextSibling.innerText.split(','))
  let finalArray = []
...
choosePicture.addEventListener('change', (event) => {
  const personalPicture = document.getElementById('personal-picture')
  personalPicture.src = URL.createObjectURL(event.target.files[0]);
});
...
const createClickEvent = (event) => {
  ...
  if (parent.classList.contains('rated')) {
    ...
    hangLoose.innerHTML = '<span>&#128077</span>'
